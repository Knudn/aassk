...
def get_event():
    ...
    for row in cur.execute('SELECT C_VALUE FROM TPARAMETERS WHERE C_PARAM IN ("EVENT", "HEAT");'):
        con.commit()
        event_values.append(row[0])
    print(event_values)
    
    if int(event_values[0]) < 9:
        num = "00"+event_values[0]
        return "Event"+num+".scdb", "Event"+num+"Ex.scdb", event_values[1]
    elif int(event_values[0]) < 99:
        num = "0"+event_values[0]
        return "Event"+num+".scdb", "Event"+num+"Ex.scdb", event_values[1]
    else:
        num = row[0]
        return "Event"+num+".scdb", "Event"+num+"Ex.scdb", event_values[1]
...
def get_start_list(eventfile, heat):
    ...
    cur = con.cursor()
    if int(heat) == 1:
        query_startlist = "SELECT C_NUM FROM TSTARTLIST_PARQ2_HEAT1;"
        query_timedata = "SELECT C_NUM, C_TIME, C_STATUS FROM TTIMEINFOS_HEAT1"
    elif int(heat) == 2:
        query_startlist = "SELECT C_NUM FROM TSTARTLIST_PARQ2_HEAT2;"
        query_timedata = "SELECT C_NUM, C_TIME, C_STATUS FROM TTIMEINFOS_HEAT2"

    for row in cur.execute(query_startlist):
        startlist.append(row)
    for row in cur.execute(query_timedata):
